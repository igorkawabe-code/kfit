...
import { getDisplayName, getTextColor } from '../lib/utils'
                ...
                <div className="flex items-end justify-center gap-3 h-40">
                  ...
                  {ranking[1] && (
                    <div className="flex flex-col items-center gap-2 flex-1">
                      ...
                      <div className="text-zinc-400 font-body text-xs font-semibold">{getFirstName(ranking[1].name)}</div>
                  ...
                  {ranking[0] && (
                    <div className="flex flex-col items-center gap-2 flex-1">
                      <div className="w-14 h-14 rounded-full flex items-center justify-center font-display text-2xl text-inherit ring-2 ring-accent ring-offset-2 ring-offset-[#060606]"
                        style={{ backgroundColor: ranking[0].color || '#B4FF00' }}>
                        {getFirstName(ranking[0].name)?.[0]}
                      </div>
                      <div className="text-white font-body text-xs font-bold">{getFirstName(ranking[0].name)}</div>
                  ...
                  {ranking[2] && (
                    <div className="flex flex-col items-center gap-2 flex-1">
                      ...
                      <div className="text-zinc-400 font-body text-xs font-semibold">{getFirstName(ranking[2].name)}</div>
            ...
            <div className="space-y-3">
              {ranking.map((p, i) => (
                <div key={p.id} className={`card flex items-center gap-4 ${p.id === profile?.id ? 'border-zinc-600' : ''}`}>
                  ...
                  <div className="w-10 h-10 rounded-full flex items-center justify-center font-display text-lg text-inherit flex-shrink-0"
                    style={{ backgroundColor: p.color || '#B4FF00' }}>
                    {getFirstName(p.name)?.[0]}
                  </div>
                  <div className="flex-1">
                    <div className={`font-body font-semibold ${p.id === profile?.id ? 'text-white' : 'text-zinc-300'}`}>
                      {p.name} {p.id === profile?.id && <span className="text-xs text-zinc-500">(você)</span>}
                    </div>
                    {/* Progress bar */}
